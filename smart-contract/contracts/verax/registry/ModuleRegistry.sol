...
contract ModuleRegistry is OwnableUpgradeable {
	...
	modifier onlyIssuers(address issuer) {
		bool isIssuerRegistered = PortalRegistry(router.getPortalRegistry()).isIssuer(
			issuer
		);
	...
	function register(
		...
		if (
			!ERC165CheckerUpgradeable.supportsInterface(
				moduleAddress,
				type(AbstractModule).interfaceId
			)
	...
	function runModules(
		...
		if (modulesAddresses.length != validationPayloads.length)
			revert ModuleValidationPayloadMismatch();
		...
		for (uint32 i = 0; i < modulesAddresses.length; i++) {
			...
			AbstractModule(modulesAddresses[i]).run(
				attestationPayload,
