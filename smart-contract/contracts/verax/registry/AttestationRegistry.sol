...
contract AttestationRegistry is OwnableUpgradeable, ERC1155Upgradeable {
	...
	modifier onlyPortals(address portal) {
		bool isPortalRegistered = PortalRegistry(router.getPortalRegistry()).isRegistered(
			portal
		);
	...
	function attest(
		...
		if (attestationPayload.attestationData.length == 0)
			revert AttestationDataFieldEmpty();
		// Auto increment attestation counter
		attestationIdCounter++;
		bytes32 id = bytes32(uint256(attestationIdCounter));
		// Create attestation
	...
	function revoke(bytes32 attestationId) public {
		...
		if (msg.sender != attestations[attestationId].portal)
			revert OnlyAttestingPortal();
		if (!isRevocable(attestations[attestationId].portal))
			revert AttestationNotRevocable();
